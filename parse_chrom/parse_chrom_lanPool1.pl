...
my $n=1;
foreach my $d(@d){
    if ($d=~/(lanSW\d+\_\d+)\.chrom/){
         my $f2 = $1."\.chrom\.txt\.gz";
         print OUT "bsub \"python parse_chrom_mzML_v08.py ../3R_reference_samples_lanPool_1.txt $d ../160308_final_lan_lib1_allFrag.csv $f2 NA\"\n";
    }
    elsif ($d=~/^(lanSW\d+\_ms1)\.chrom/){
         my $f2 = $1."\.chrom\.txt\.gz";
         print OUT "bsub \"python parse_chrom_mzML_v08.py ../3R_reference_samples_lanPool_1.txt $d ../160308_final_lan_lib1_allFragMS1.csv $f2 NA\"\n";
    }
    $n++;
    if ($n%1000==0){
         print OUT "sleep 1m\n";
