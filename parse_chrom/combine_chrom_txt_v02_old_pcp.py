...
title = ['transition_name','transition_group_id', 'best_rt', 'best_sample', 'best_score', 'protein', 'irt', 'precursor_mz', 'transition_mz', 'transition_int']
title2 = ['transition_name','transition_group_id', 'best_rt', 'best_sample', 'best_score', 'protein', 'irt', 'precursor_mz', 'transition_mz', 'transition_i']
    ...
    with gzip.open(in_file, 'rb') as gz:
        ...
        for row in reader:
            if 'transition_int' in row:
                ...
                d[transition]['i_list'][i] = row['int_list']
            elif 'transition_i' in row:
                transition = row[title2[0]]
                tg.append(row[title2[1]])
                for j in range(1, 10):
                    d[transition][title2[j]] = row[title2[j]]
                if (d[transition]['best_sample'].lower() in id_mapping):
                    d[transition]['best_sample'] = id_mapping[d[transition]['best_sample'].lower()]
                d[transition]['rt_list'][i] = row['rt_list']
