...
id_mapping_file = sys.argv[5]  #'goldenSets90.txt'  if no need to use id mapping, use NA here
...
def read_reference_peak_group(id_mapping):
    ...
    with open(in_file1) as CSV_FILE1:
        ...
        for row in reader:
            ...
            tg_name_id = ''
            if 'tg_name' in row.keys():
                tg_name_id = 'tg_name'
            elif 'fragment_name' in row.keys():
                tg_name_id = 'fragment_name'

            best_rt[row[tg_name_id]] = float(row['reference_rt'])
            if bool(id_mapping):
                best_sample[row[tg_name_id]] = id_mapping[row['reference_sample'].lower()]
            else:
                best_sample[row[tg_name_id]] = row['reference_sample']
            best_score[row[tg_name_id]] = float(row['reference_score'])
...
def read_id_mapping_file():
    ...
    with open(id_mapping_file, 'rb') as MAPPING_FILE:
        ...
        for row in reader:
            id_mapping[row[0].lower()] = row[1].lower()
