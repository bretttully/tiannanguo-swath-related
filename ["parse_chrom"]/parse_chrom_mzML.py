...
def read_reference_peak_group(id_mapping):
    ...
    with open(in_file1) as CSV_FILE1:
        ...
        for row in reader:
            ...
            best_rt[row['tg_name']] = float(row['reference_rt'])
            if bool(id_mapping):
                best_sample[row['tg_name']] = id_mapping[row['reference_sample'].lower()]
            else:
                best_sample[row['tg_name']] = row['reference_sample']
            best_score[row['tg_name']] = float(row['reference_score'])
                        ...
                        for binary_array_data in binary_array_data_list.iter():
                            ...
                            for binary in binary_array_data.iter(binary_tag):
                                if if_rt_list == 1:
                                    ...
                                    #decode rt and int
                                    if rt_list == None:
                                        print 'no value ', rt_list
                                        writer.writerow((transition_name, 'NA', 'NA',
