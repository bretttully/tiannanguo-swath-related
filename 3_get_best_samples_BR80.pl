...
$in="br304sw.txt";
$in1="2R_pep_mscore.txt";
$in2="2R_pep_rt.txt";
$o="3R_reference_samples_BR80.txt";
...
open(OUT,">$o");
print OUT "tg_name\treference_sample\treference_score\treference_rt\n";
foreach my $j(1..$#d1){
          my $d=$d1[$j];
          my @dd=&s($d);
          my $tg=$dd[0];
          ...
          my $i2;
          foreach my $i (1..$#dd){
                   if ($dd[$i]=~/\d/ and $dd[$i]<$mscore and $sw{$t1[$i]}){
                        $mscore=$dd[$i];
                        $sample=$t1[$i];
                        $i2=$i;
                   }
