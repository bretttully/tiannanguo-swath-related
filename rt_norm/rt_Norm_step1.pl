...
$in=shift;
$in2=shift; #"swath_wnd_br66.txt";
$iRT_lib="iRT_library.TraML";
$o="rt_norm_step1.sh";
@d=&oF($in);

@d2=&oF($in2);
my $swWindows_num=scalar(@d2);

my $n=1;

open(OUT,">$o");
print OUT "mkdir rt_norm\n" ;
foreach my $d(@d){
          my @dd=&s($d);              
          foreach my $i (1..$swWindows_num){
                  print OUT "bsub OpenSwathChromatogramExtractor -is_swath -in $dd[1]/$dd[1]\_$i.mzML.gz -tr $iRT_lib -out rt_norm/$dd[1]\_$i\.rtnorm\.chrom\.mzML -rt_extraction_window -1\n";
                  $n++;
                  if ($n%500==0){
                       print OUT "\nsleep 30m\n\n";
                  }
