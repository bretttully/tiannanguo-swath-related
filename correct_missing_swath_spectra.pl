...
$in=shift;
$o=shift;
#$o=~s/\.mzML/\_corrected\.mzML/;
#"BR1_23.mzML.gz";
#$o="BR1_23_corrected.mzML.gz";

tie *OUT, 'IO::Zlib', $o, "wb";
tie *IN, 'IO::Zlib', $in, "rb";

my $if_spectra=0;
my $if_reading_buffer=0;

my $first_window=-1;
my $first_window_lower=-1;
my $first_window_upper=-1;
my $buffer;

while (my $d=<IN>){
     if ($if_spectra==0 && $d!~/\<spectrum id\=\"scan\=1\"/){
          print OUT $d;
     }

     if ($d=~/\<spectrum id\=\"scan\=\d+\"/){
          $if_spectra=1;
          $buffer.=$d;
          $if_reading_buffer=1;
     }
     elsif ($d!~/\<\/spectrum\>/ && $if_reading_buffer ==1){
          $buffer.=$d;
     }
     elsif ($d=~/\<\/spectrum\>/){
             $buffer.=$d;
             my ($this_window, $this_window_lower, $this_window_upper)=&get_window($buffer);
             if ($first_window==-1){  #for the first spectrum
                  $first_window=$this_window;
                  $first_window_lower=$this_window_lower;
                  $first_window_upper=$this_window_upper;
             }
             else{
                  if ($this_window ne $first_window){
                          my $buffer2=&process_buffer($buffer,$first_window, $first_window_lower, $first_window_upper);
                          print OUT $buffer2;
                          $buffer="";
                          $if_reading_buffer=0;
                  }
...
sub process_buffer{
    my ($buffer,$this_window, $this_window_lower, $this_window_upper)= @_;
    $buffer=~s/name\=\"isolation window target m\/z\" value\=\"[0-9.]+\"/name\=\"isolation window target m\/z\" value\=\"$this_window\"/;
    $buffer=~s/name\=\"isolation window lower offset\" value\=\"[0-9.]+\"/name\=\"isolation window lower offset\" value\=\"$this_window_lower\"/;
    $buffer=~s/name\=\"isolation window upper offset\" value\=\"[0-9.]+\"/name\=\"isolation window upper offset\" value\=\"$this_window_upper\"/;
    $buffer=~s/name\=\"selected ion m\/z\" value\=\"[0-9.]+\"/name\=\"selected ion m\/z\" value\=\"$this_window\"/;
    $buffer=~s/\<binaryDataArray encodedLength\=\"\d+\"\>/\<binaryDataArray encodedLength\=\"0\"\>/g;
    $buffer=~s/\<binary\>.*\<\/binary\>/\<binary\>\<\/binary\>/g;
    return $buffer;
}

sub get_window{
    my $b=shift;
    $b=~/name\=\"isolation window target m\/z\" value\=\"([0-9.]+)\"/;
    my $w1=$1;
    $b=~/name\=\"isolation window lower offset\" value\=\"([0-9.]+)\"/;
    my $w2=$1;
    $b=~/name\=\"isolation window upper offset\" value\=\"([0-9.]+)\"/;
    my $w3=$1;
    return ($w1, $w2, $w3);
}
